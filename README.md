...
```bash
   git clone [https://github.com/DeveloperKauan/EstruturaDados.git](https://github.com/DeveloperKauan/EstruturaDados.git)
   ```
...
> **Sempre** usar git pull antes de começar a escrever código para garantir que está com o projeto atualizado e não dar B.O:
> ```bash
...
Quando terminar as alterações, esses são os comandos pra salvar e enviar seu progresso:
...
> **Atenção:** Não fazer commits com mensagens genéricas tipo `"consertar/fix"`, `"mudar X coisa"` ou `"atualizando"`. 
Isso tira ponto na nota individual
---

## Regras Importantes

1. Cada um commita estritamente na **sua própria branch**, nunca direto na `main`.
